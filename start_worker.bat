...
if exist "%SERVER_DIR%\dist\beatsync-worker.exe" (
  start "BeatSync Hybrid Worker" cmd /k "cd /d ""%SERVER_DIR%"" && set ""BEATSYNC_ENV_FILE=.env.worker"" && .\dist\beatsync-worker.exe"
) else (
  where go >nul 2>nul
  if errorlevel 1 (
    echo Khong tim thay dist\beatsync-worker.exe hoac Go trong PATH.
    exit /b 1
  )
